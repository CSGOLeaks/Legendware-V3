    ...
    for (int i{ 1 }; i < m_globals()->m_maxclients; ++i) {
        ...
        if (weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE) {
            ...
            if (ptr.hit_entity == player) {
                ...
                // clip max damage.
                dmg = min(dmg, (player->m_ArmorValue() > 0) ? 57.f : 98.f);
    ...
    if (type && path.size() > 1)
    {
        ...
        for (auto it = others.begin(); it != others.end(); it++)
        {
            ...
            if (math::world_to_screen(it->first, screen))
                render::get().rect(screen.x - 2, screen.y - 2, 4, 4, it->second);
        }
        ...
        if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
            render::get().Draw3DCircleFilled(endpos, 144, g_cfg.esp.grenade_prediction_tracer_color);
        else if (weapon->m_iItemDefinitionIndex() == WEAPON_MOLOTOV || weapon->m_iItemDefinitionIndex() == WEAPON_INCGRENADE)
            render::get().Draw3DCircleFilled(endpos, 150, g_cfg.esp.grenade_prediction_tracer_color);
    }
}
...
void GrenadePrediction::Simulate(CViewSetup* setup)
{
    ...
    path.push_back(vecSrc);
    others.push_back(std::make_pair(vecSrc, Color::Red));
}
...
int GrenadePrediction::Step(Vector& vecSrc, Vector& vecThrow, int tick, float interval)
{
    ...
    if (tr.fraction != 1.0f)
    {
        ...
        others.push_back(std::make_pair(tr.endpos, Color::White));
    }
...
void GrenadePrediction::ResolveFlyCollisionCustom(trace_t& tr, Vector& vecVelocity, Vector move, float interval)
{
    if (tr.hit_entity)
    {
        if (false)//autowall::get().is_breakable_entity(tr.hit_entity))
        {
