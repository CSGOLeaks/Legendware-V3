...
bool weapon_t::can_fire(bool check_revolver)
{
	...
	if (owner == g_ctx.local() && (g_cfg.antiaim.hide_shots && g_cfg.antiaim.hide_shots_key.key > KEY_NONE && g_cfg.antiaim.hide_shots_key.key < KEY_MAX && misc::get().hide_shots_key || g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key && !g_cfg.ragebot.instant && g_ctx.globals.weapon->can_double_tap()) && (g_ctx.globals.tickbase_shift || g_ctx.globals.next_tickbase_shift))
	{
		auto server_time = g_ctx.globals.fixed_time;

		if (g_ctx.globals.tickbase_shift)
			server_time -= TICKS_TO_TIME(g_ctx.globals.tickbase_shift);
		else
			server_time -= TICKS_TO_TIME(g_ctx.globals.next_tickbase_shift + 1);
	...
	else if (owner == g_ctx.local() && g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key && g_cfg.ragebot.instant && g_ctx.globals.weapon->can_double_tap())
	{
	...
	{
		if (g_ctx.globals.fixed_time < m_flNextPrimaryAttack())
			return false;

		if (g_ctx.globals.fixed_time < owner->m_flNextAttack())
			return false;

		if (check_revolver && m_iItemDefinitionIndex() == WEAPON_REVOLVER && m_flPostponeFireReadyTime() >= g_ctx.globals.fixed_time)
			return false;
	}
...
bool weapon_t::can_shift_tickbase(int tickbase_shift)
{
	if (!this) //-V704
		return false;

	auto server_time = g_ctx.globals.fixed_time - TICKS_TO_TIME(tickbase_shift);

	if (server_time < g_ctx.local()->m_flNextAttack())
		return false;
...
bool weapon_t::can_double_tap()
{
	...
	if (idx == WEAPON_TASER || idx == WEAPON_REVOLVER || idx == WEAPON_SSG08 || idx == WEAPON_AWP)
		return false;
...
std::array <float, 24>& player_t::m_flPoseParameter()
{
...
Vector player_t::get_shoot_position()
{
	...
	if (this != g_ctx.local() || this == g_ctx.local() && !fakelag::get().condition && key_binds::get().get_key_bind_state(20)) //-V648
		return GetAbsOrigin() + m_vecViewOffset();

	Vector eye_pos;
	call_virtual <void(__thiscall*)(void*, Vector&)>(this, g_ctx.indexes.at(12))(this, eye_pos);

	if (m_vecViewOffset().z > 64.0f)
		eye_pos.z = eye_pos.z - m_vecViewOffset().z + 64.0f;
	else if (m_vecViewOffset().z < 46.099998f)
		eye_pos.z = eye_pos.z - m_vecViewOffset().z + 46.0f;

	return eye_pos;
}

Vector player_t::get_shoot_position(float viewOffsetZ)
{
	if (!this) //-V704
		return ZERO;

	Vector viewOffset = m_vecViewOffset();
	viewOffset.z = viewOffsetZ;
...
bool player_t::SetupBonesFixed(matrix3x4_t* matrix)
{
	if (!this) //-V704
		return false;
	...
	*(uint8_t*)((uintptr_t)this + 0x274) = 0;
	...
	if (SetupBones(matrix, matrix ? MAXSTUDIOBONES : -1, BONE_USED_BY_ANYTHING, m_flSimulationTime()))
		setuped = true;
